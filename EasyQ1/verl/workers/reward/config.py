...
class RewardConfig:
    ...
    score_function_batch: bool = False
    """Call the custom function once for the complete rollout batch.

    Batch functions receive ``(predict_strs, ground_truths)`` and return one
    score dictionary per response.  This is required for expensive external
    rewards such as TTRL voting: the five GRPO candidates must be evaluated in
    one request instead of issuing five independent model calls.
    """
