...
class TensorBoardLogger(Logger):
    def __init__(self, config: Dict[str, Any]) -> None:
        # Keep console-only runs independent from the optional TensorBoard
        # backend. Import it only when this logger is actually selected.
        ...
        # TensorBoard hparams accept only scalar primitives. Hydra/OmegaConf
        # fields such as logger lists and null checkpoint paths are preserved
        # as readable strings instead of aborting logger initialization.
        hparams = {
...
class WandbLogger(Logger):
    def __init__(self, config: Dict[str, Any]) -> None:
        # This cluster has no outbound W&B access.  Force offline mode even if
        # the caller forgot to source augmentation_env.sh, and keep all files
        # in the explicitly selected repository-local directory.
        wandb_dir = os.getenv("WANDB_DIR", "wandb_log")
        os.makedirs(wandb_dir, exist_ok=True)
        os.environ["WANDB_MODE"] = "offline"
        print(f"Saving offline W&B run to {wandb_dir}.")
        wandb.init(
