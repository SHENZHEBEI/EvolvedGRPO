...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        available_steps = len(train_dataloader) * config.trainer.total_episodes
        if config.trainer.max_steps is not None:
            if config.trainer.max_steps > available_steps:
                raise ValueError(
                    "Requested max_steps exceeds the filtered dataset capacity: "
                    f"max_steps={config.trainer.max_steps}, available_steps={available_steps}, "
                    f"batches_per_episode={len(train_dataloader)}, "
                    f"total_episodes={config.trainer.total_episodes}. "
                    "Inspect overlong-prompt filtering before allocating GPU workers."
                )
            self.training_steps = config.trainer.max_steps
        else:
            self.training_steps = available_steps
    ...
    def fit(self):
        ...
        for _ in tqdm(range(self.config.trainer.total_episodes), desc="Episode", position=0):
            for batch_dict in tqdm(self.train_dataloader, desc="Running step", position=1):
                # Check before incrementing so max_steps=1 performs and saves
                # exactly global_step_1 rather than advancing the counter to 2
                # while breaking on the next dataloader batch.
        ...
        # val_freq <= 0 is documented as disabling validation.  Do not perform
        # an implicit final pass: an online TTRL/image reward can be more
        # expensive than the complete actor update and validation n=1 is not a
        # meaningful GRPO group anyway.
