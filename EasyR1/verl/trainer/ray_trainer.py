    ...
    def fit(self):
        ...
        for _ in tqdm(range(self.config.trainer.total_episodes), desc="Episode", position=0):
            for batch_dict in tqdm(self.train_dataloader, desc="Running step", position=1):
                # Do not advance the counter on the iterator item used only to
                # discover that training is complete.  For the fixed 15616-row
                # dataset this preserves exactly 30 updates and global_step_30.
